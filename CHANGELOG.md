## 🌌 Freyja v1.0.12 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- Actualizar build.gradle.kts
- Actualizar android-release-signed.yml
- docs(changelog): update for v0.9.13
- ci: step to compile Kotlin (debug) and upload logs
- docs(changelog): update for v0.9.12
- fix: ComponentActivity + android.app.AlertDialog; add activity-ktx/core-ktx
- docs(changelog): update for v0.9.11
- ci: purge caches + strong gradle flags (+Xmx6g, no-daemon, 1 worker, refresh-deps)
- ci: +RAM 6G, daemon off, 1 worker, AAPT2 single thread
- docs(changelog): update for v0.9.10
- deps: add appcompat 1.7.0 + activity-ktx 1.9.2
- ci: workers=1
- docs(changelog): update for v0.9.9
- docs(changelog): update for v0.9.8
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.13 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- ci: step to compile Kotlin (debug) and upload logs
- docs(changelog): update for v0.9.12
- fix: ComponentActivity + android.app.AlertDialog; add activity-ktx/core-ktx
- docs(changelog): update for v0.9.11
- ci: purge caches + strong gradle flags (+Xmx6g, no-daemon, 1 worker, refresh-deps)
- ci: +RAM 6G, daemon off, 1 worker, AAPT2 single thread
- docs(changelog): update for v0.9.10
- deps: add appcompat 1.7.0 + activity-ktx 1.9.2
- ci: workers=1
- docs(changelog): update for v0.9.9
- docs(changelog): update for v0.9.8
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.12 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- fix: ComponentActivity + android.app.AlertDialog; add activity-ktx/core-ktx
- docs(changelog): update for v0.9.11
- ci: purge caches + strong gradle flags (+Xmx6g, no-daemon, 1 worker, refresh-deps)
- ci: +RAM 6G, daemon off, 1 worker, AAPT2 single thread
- docs(changelog): update for v0.9.10
- deps: add appcompat 1.7.0 + activity-ktx 1.9.2
- ci: workers=1
- docs(changelog): update for v0.9.9
- docs(changelog): update for v0.9.8
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.11 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- ci: purge caches + strong gradle flags (+Xmx6g, no-daemon, 1 worker, refresh-deps)
- ci: +RAM 6G, daemon off, 1 worker, AAPT2 single thread
- docs(changelog): update for v0.9.10
- deps: add appcompat 1.7.0 + activity-ktx 1.9.2
- ci: workers=1
- docs(changelog): update for v0.9.9
- docs(changelog): update for v0.9.8
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.10 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- deps: add appcompat 1.7.0 + activity-ktx 1.9.2
- ci: workers=1
- docs(changelog): update for v0.9.9
- docs(changelog): update for v0.9.8
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.9 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- ci: workers=1
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.8 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- ci: gradle.properties (memoria, workers, AAPT2 single thread)
- ci: permitir gradle.properties en repo
- docs(changelog): update for v0.9.7
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.7 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- bridge: asegurar carga nativa (System.loadLibrary("llama"))
- docs(changelog): update for v0.9.6
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.6 — Guardia Activa (2025-08-31)

### ✨ Cambios:
- ui: StandaloneActivity 100% programática (sin XML) para evitar fallos de recursos
- docs(changelog): update for v0.9.5
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.5 — Guardia Activa (2025-08-30)

### ✨ Cambios:
- deps: add Material 1.12.0 for Theme.Material3
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- docs(changelog): update for v0.9.4
- docs(changelog): update for v0.9.3
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.4 — Guardia Activa (2025-08-30)

### ✨ Cambios:
- fix(theme): usar Theme.Freyja y limpiar styles
- fix(theme): Manifest usa Theme.Freyja (no Material3.DayNight)
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.3 — Guardia Activa (2025-08-30)

### ✨ Cambios:
- Actualizar themes.xml
- Crear android-release-signed.yml
- Crear native-lib.cpp
- Crear CMakeLists.txt
- docs(changelog): update for v0.9.1
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

## 🌌 Freyja v0.9.1 — Guardia Activa (2025-08-30)

### ✨ Cambios:
- Actualizar StandaloneActivity.kt
- Crear ModelStorage.kt
- Actualizar build.gradle.kts
### 🔮 Mejoras Futuras:
- Integrar memoria persistente para entrenamientos de combate.
- Añadir hotword local ("Despierta, Freyja").
- Temas intercambiables: grimorio rúnico / templo estelar / altar de fuego.
- Reducir tamaño del APK: shrinkResources + splits por ABI.

---

